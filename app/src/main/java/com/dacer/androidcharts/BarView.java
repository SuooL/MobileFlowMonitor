...
//    private boolean showSideMargin = true;
    ...
    private final int FOREGROUND_COLOR = Color.parseColor("#2980b9");
...
//        this.dataList = null;
     ...
     */
    public void setDataList(ArrayList<Double> list, int max){
        targetPercentList = new ArrayList<Double>();
        dataList = list;

        if(max == 0) max = 1;

        for(Double integer : list){
            targetPercentList.add(1-(double)integer/(double)max);
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Paint textRecPaint = new Paint( Paint.ANTI_ALIAS_FLAG);
        textRecPaint.setTextSize(30);
        textRecPaint.setColor( Color.GREEN);
        textRecPaint.setTextAlign(Paint.Align.CENTER);

        if(percentList != null && !percentList.isEmpty()){
            for(Double f: percentList){
                ...
                if(drawText){
                    ...
                    canvas.drawText(String.valueOf(df.format(dataList.get(i-1)))+"M",
                            (BAR_SIDE_MARGIN*i+barWidth*(i-1)+(BAR_SIDE_MARGIN+barWidth)* i)/2,
                            topMargin+(int)((getHeight()-topMargin-bottomTextHeight-TEXT_TOP_MARGIN)*percentList.get(i-1))-25,
                            textRecPaint);
                }
    ...
    public void setDrawTest(boolean drawText) {
